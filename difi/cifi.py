...
from .utils import _checkColumnTypes, _classHandler
...
def analyzeObservations(
    ...
    **metric_kwargs
    ...
    dtypes = np.dtype([(truth_col, str), ("num_obs", int), ("findable", int)])
    ...
    if detection_window is not None:
        ...
        for night in night_range:
            ...
            win_obs = observations[
                (
                    (observations[column_mapping["night"]] >= night)
            ...
            window_findable_observations = metric_func(
                win_obs, column_mapping=column_mapping, **metric_kwargs
            )
